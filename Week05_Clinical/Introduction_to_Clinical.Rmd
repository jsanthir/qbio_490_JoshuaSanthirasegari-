...
colnames(mtcars) 
rownames(mtcars) 
mtcars$carbs # fill in name of carburetor column here
```
...
```{r}
write.csv(clinic, "/home1/jsanthir/490_cluster/analysis_data1/brca_clinical_data.csv", row.names=FALSE)
```

To read the csv back into your environment as a dataframe:

```{r}
clinic <- read.csv("/home1/jsanthir/490_cluster/analysis_data1/brca_clinical_data.csv")
```
...
```{r Exercise 4.2}
# create plot here
clinic$lymph_nodes_examined_he_count <- as.numeric(clinic$lymph_nodes_examined_he_count)
clinic$her2_ihc_score <- as.factor(clinic$her2_ihc_score)
boxplot(clinic$lymph_nodes_examined_he_count ~ clinic$her2_ihc_score, 
        main = "Lymph Nodes Examined by HER2 IHC Score", 
        xlab = "HER2 IHC Score", 
...
Survival probability, which is how likely the patient is to survive after a certain number of days pass. 

# 2. What does the x axis represent?
Time in days which demonstrates how long the patient is alive to determine how likley it is they will survive at that time. 
...
The p-value is 0.2, which is not statistically significant as it is not less than 0.05. This demonstrates there is no significant relationship between the age that a patient is diagnosed and their survival rate in this set of breast cancer patients. 
